...
class StashPerformer:
    ...
    def parse_measurements(self):
        ...
        if band_cup_waist_hips := re.match(
            r"^(?P<band>\d+)(?P<cupsize>[a-zA-Z]+)\-(?P<waist>\d+)\-(?P<hips>\d+)$",
            self.measurements,
        ):
            m = band_cup_waist_hips.groupdict()

        # Fashion Measurements | Bust, Waist, Hips | Example: "36-28-34"
        elif bust_waist_hips := re.match(
            r"^(?P<bust>\d+)\-(?P<waist>\d+)\-(?P<hips>\d+)$", self.measurements
        ):
            m = bust_waist_hips.groupdict()

        # Bra Measurements | Band, Cup Size | Example: "32D", "32D (81D)", "32D-??-??"
        elif band_cup := re.match(
            r"^(?P<band>\d+)(?P<cupsize>[a-zA-Z]+)", self.measurements
        ):
        ...
        if (
            ...
            log.debug(
                f"converted measurements from metric {self.measurements} -> {self.band}{self.cupsize}-{self.waist}-{self.hips}"
            )
        elif (
            self.bust
            and self.bust > 50
            ...
            self.hips = self.hips / CM_TO_INCH
            log.debug(
                f"converted measurements from metric {self.measurements} -> {self.bust}-{self.waist}-{self.hips}"
            )
        ...
        if self.band and self.cupsize:
            ...
            log.debug(
                f"Bra size {int(self.band)}{self.cupsize} converted to {(self.band / 2.0)} + {self.bust_band_diff} = {self.breast_volume} volume points"
            )
    ...
    def match_body_shapes(self):
        ...
        if not self.body_shapes:
            if not self.bust or not self.waist or not self.hips:
                log.debug(
                    f"{self}: could not classify bodyshape, missing required measurements"
                )
            else:
                log.warning(
                    f"{self}: could not classify bodyshape bust={self.bust:.0f} waist={self.waist:.0f} hips={self.hips:.0f}"
                )
    ...
    def set_type_descriptor(self):
        ...
        if descriptor == BodyType.FIT and HeightType.SHORT.within_threshold(
            self.height_cm
        ):
            descriptor = BodyType.PETITE
        if (
            descriptor == BodyType.AVERAGE
            and self.body_shapes
            and any(bs in self.body_shapes for bs in CURVY_SHAPES)
        ):
