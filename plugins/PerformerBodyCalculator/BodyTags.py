...
class PBCError(StashTagEnum):
    BAD_MEASUREMENTS = StashTagDC(
        description="performer measurements could not be parsed by PBC"
    )
    NO_BODYSHAPE_MATCH = StashTagDC(
        description="Could not determine bodyshape from given measurements"
    )
...
CURVY_SHAPES = [
    BodyShape.TOP_HOURGLASS,
    BodyShape.BOTTOM_HOURGLASS,
    BodyShape.HOURGLASS,
]
...
def approximate_breast_weight(bust_band_diff):
    ...
    weight_lb = (
        0.765
        + 0.415 * bust_band_diff
        + -0.0168 * bust_band_diff**2
        + 0.00247 * bust_band_diff**3
    )
